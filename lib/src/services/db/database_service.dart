...
class DatabaseService {
  ...
  Future<Map<String, dynamic>> backupProductsAndCategories() async {
    try {
      // الحصول على مجلد المستندات
      final Directory appDir = await getApplicationDocumentsDirectory();
      final String backupDir = p.join(appDir.path, 'backups');
      ...
      final String fileName = 'products_categories_backup_$timestamp.json';
      final String filePath = p.join(backupDir, fileName);
      ...
      final List<Map<String, dynamic>> categories =
          await _db.rawQuery('SELECT * FROM categories');
      ...
      final Map<String, dynamic> backupData = {
        'backupType': 'products_and_categories',
      ...
      // كتابة البيانات إلى الملف كـ JSON
  ...
  /// إنشاء نسخة احتياطية للمنتجات والأقسام في مسار محدد
  Future<Map<String, dynamic>> backupProductsAndCategoriesToPath(
    ...
    try {
      ...
      final Directory parentDir = backupFile.parent;
      ...
      if (!await backupFile.exists()) {
        return {
          'success': false,
          'error': 'فشل في إنشاء ملف النسخة الاحتياطية',
        };
      }
      ...
      return {
        'success': false,
        'error': 'حدث خطأ أثناء إنشاء النسخة الاحتياطية: ${e.toString()}',
      };
    }
  }

  /// استعادة المنتجات والأقسام من ملف النسخة الاحتياطية
  Future<Map<String, dynamic>> restoreProductsAndCategories(
      String filePath) async {
    try {
      ...
      final String fileContent = await backupFile.readAsString(encoding: utf8);
      final Map<String, dynamic> backupData = jsonDecode(fileContent);

      // التحقق من نوع النسخة الاحتياطية
      if (backupData['backupType'] != 'products_and_categories') {
        return {
          'success': false,
          'error': 'هذا الملف ليس نسخة احتياطية للمنتجات والأقسام',
        };
      }
      ...
      await _db.transaction((txn) async {
        ...
        // خريطة لربط الـ ID القديم بالجديد للأقسام
        final Map<int, int> categoryIdMap = {};

        // استعادة الأقسام أولاً
        final List<dynamic> categories = backupData['categories'] ?? [];
        for (final Map<String, dynamic> category in categories) {
          // حفظ الـ ID القديم
          final int oldId = category['id'] ?? 0;

          // إزالة الـ id لتجنب التضارب
          final Map<String, dynamic> categoryData = Map.from(category);
          categoryData.remove('id');

          // إدراج القسم والحصول على الـ ID الجديد
          final int newId = await txn.insert('categories', categoryData);

          // ربط الـ ID القديم بالجديد
          categoryIdMap[oldId] = newId;
        }

        // استعادة المنتجات مع تحديث category_id
        final List<dynamic> products = backupData['products'] ?? [];
        for (final Map<String, dynamic> product in products) {
          // إزالة الـ id لتجنب التضارب
          final Map<String, dynamic> productData = Map.from(product);
          productData.remove('id');

          // تحديث category_id إذا كان موجوداً
          if (productData['category_id'] != null) {
            final int oldCategoryId = productData['category_id'];
            if (categoryIdMap.containsKey(oldCategoryId)) {
              productData['category_id'] = categoryIdMap[oldCategoryId];
            } else {
              // إذا لم يتم العثور على القسم، إزالة category_id
              productData['category_id'] = null;
            }
          }

          await txn.insert('products', productData);
        }
      });

      return {
        'success': true,
        'categoriesRestored': backupData['categories']?.length ?? 0,
      ...
      return {
        'success': false,
        'error': 'حدث خطأ أثناء استعادة البيانات: ${e.toString()}',
      };
    }
  }

  /// إنشاء نسخة احتياطية شاملة لجميع البيانات
  Future<Map<String, dynamic>> createFullBackup(String filePath) async {
    try {
      final File backupFile = File(filePath);

      // إنشاء نسخة من قاعدة البيانات الحالية
      await File(_dbPath).copy(filePath);
  ...
  Future<Map<String, dynamic>> restoreFullBackup(String filePath) async {
    try {
      final File backupFile = File(filePath);
      if (!await backupFile.exists()) {
        return {
      ...
      try {
        ...
        // تجاهل خطأ إعادة الفتح
      }

      return {
        'success': false,
        'error': 'حدث خطأ أثناء استعادة النسخة الاحتياطية: ${e.toString()}',
      };
    }
  }
}
